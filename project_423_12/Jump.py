...
def drawScore():
    global pause, over
    if over == 1:
        draw_text(-135, -33, "Game Over!! Go Again?")
    if pause and not over:
        draw_text(-95, -33, "Ball Jump!")
        draw_text(-85, -54, "Paused!")
...
gravity = 0.010  # Acceleration due to gravity (adjust as needed for smoother motion)
...
def animate_ball():
    ...
    if page == 4 and phase == 2:
        ...
        if life == 0:
            ...
            print('Game Over')
            over = 1
        if ((ball_x - e6[0]) ** 2 + (ball_y - e6[1]) ** 2) ** 0.5 <= 30 or (
                (ball_x - e4[0]) ** 2 + (ball_y - e4[1]) ** 2) ** .5 <= 30:
            life -= 1
            ball_x = -380
            ball_y = -9
            phase = 2
    ...
    if page == 2 and phase == 1:
        if life == 0:
            ...
            print('Game Over')
            over = 1
        if ((ball_x - e1[0]) ** 2 + (ball_y - e1[1]) ** 2) ** .5 <= 30 or (
                (ball_x - e2[0]) ** 2 + (ball_y - e2[1]) ** 2) ** .5 <= 30:
            life -= 1
            ball_x = 340
    ...
    if page == 2 and phase == 1:
        ...
        if (jump_state > 0 and reverse == False):  # If ball is jumping
            ...
            # Checking if ball has landed on the ground or obstacle
            if ball_y <= -163:  # Check if ball has landed on the ground
                ball_y = -163  # Set ball to ground level
                jump_state = 0  # Reset jump state
                jump_velocity = 0  # Reset jump velocity
    ...
    if page == 2 and phase == 2:
        ...
        if ball_y <= 9:
            ball_y = 9  # making sure the ball stays on the ground
        else:
            ball_y -= up
...
def checkObstacleCollision(new_x, new_y):
    ...
    if phase == 1:
        # Check if the new position is colliding with any obstacle
        if (obstacle_left_wall_started <= new_x <= obstacle_left_wall_ended) or (
            ...
            # Check collision with block height
            if new_y > -100:
                # Ball is above the  height, adjusting to the top
                ball_y = -80
                jump_checker = True
                return False
            elif new_y < -100:

                return True  # activating if Collision detected
        else:
            jump_checker = False
...
t_collected = False
t_collected2 = False
pause_duration = 10  # Duration of pausing the laser after
pause_start_time = 0  # storing the start time of the pause
...
def laser():
    ...
    if page == 2:
        ...
        if not t_collected2 and not pause:
            ...
            laser_y_start_4 -= 0.5  # upward
            laser_y_start_5 -= 0.5  #  upward
            laser_y_start_6 -= 0.5  #  upward
...
def mouseListener(button, state, x, y):
    ...
    if page == 3:
        ...
        draw_text(-53, -25, 'Start Again')
        draw_text(-45, -50, 'Quit')
...
def display():
    ...
    if page == 1:
        ...
        draw_text(-110, 150, 'Welcome to Ball Jump')
        draw_text(-90, 80, 'Choose Color')
        draw_text(300, 150, 'Continue')
    ...
    if page == 3:
        draw_text(-110, 0, 'Thank you for Trying!')
        draw_text(-53, -25, 'start again')
        draw_text(-45, -50, 'Quit')
    glutSwapBuffers()
